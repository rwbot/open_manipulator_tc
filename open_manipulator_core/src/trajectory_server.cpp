...
std::vector<double> DynamixelTrajectoryAction::calculatePoint(ros::Duration curTime) {
   while (curTime > points[idx + 1].time_from_start) idx++;
   double percent = (curTime.toSec() - points[idx].time_from_start.toSec()) / 
                     (points[idx + 1].time_from_start.toSec() - points[idx].time_from_start.toSec());  //Inverse Lerp
   ROS_INFO_STREAM("CurTime: " << curTime << 
                     "\nTime(" << idx << "): " << points[idx].time_from_start.toSec() << 
                     "\nTime(" << idx + 1 << "): " << points[idx + 1].time_from_start.toSec() << 
                     "\nPrecent: " << percent <<
                     "\n");
   std::vector<double> retval;
   for (int i = 0; i < points[idx].positions.size(); i++) {
      retval.push_back(points[idx].positions[i] + (points[idx + 1].positions[i] - points[idx].positions[i]) * percent); //Lerp
   }
   return retval;
}
